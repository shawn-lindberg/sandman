...
bool Schedule::ReadFromFile(const char* fileName)
{
	...
	{
		Logger::FormatWriteLine<Shell::ColorIndex::Red>("Failed to open the schedule file %s.\n", fileName);
		return false;
	}
	...
	{
		Logger::FormatWriteLine<Shell::ColorIndex::Red>("Failed to parse the schedule file %s.\n", fileName);
		fclose(scheduleFile);
		return false;
	}
...
static void ScheduleLogLoaded()
{
	...
	for (auto const& event : s_Schedule.GetEvents())
	{
		...
		// Print the event.
		Logger::FormatWriteLine("\t+%01ih %02im %02is -> %s, %s", delayHours, delayMin, 
